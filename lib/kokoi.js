...
function kokoi(conf) {
  ...
  conf.command = conf.command || ('pandoc -f markdown -t html5' + (conf.mathjax ? ' --mathjax' : ''));
  ...
  if (/^(?:[A-Za-z0-9]+(?:\,[A-Za-z0-9]+){0,})?$/.test(conf.extensions)) {
    ...
    conf.extensions = new RegExp('\\.(' + conf.extensions + ')$', 'gi');
  } else {
  ...
  function renderTemplate() {
    ...
    var index = template.render(layout, indexContent);
    layout = template.render(layout, templateContent);
    fs.writeFile(resdir + '/index.html', index);
  }
